...
CSession::CSession( boost::asio::io_context &io_context, CServer *server )
    : _socket( io_context ), _server( server ), _b_close( false ),
      _b_head_parse( false ), _io_context(io_context) {
...
void CSession::Start() {
    auto shared_this = SharedSelf();

    // coroutine
    co_spawn(_io_context, [=, this]()->awaitable<void>{
        try {
            for ( ; !_b_close; ) {
                ...
                // 投递给逻辑线程
                LogicSystem::GetInstance()->PostMsgToQue( make_shared<LogicNode>(
